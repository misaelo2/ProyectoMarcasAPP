...
def movimientos(cuentaid) :
	...
	if fechafrom and fechato :
		...
		json=r4.json()
		return json 
		#template("infotransacciones.tpl",cuentaid=cuentaid,movimientos=json["data"]["accountTransactions"],fechafrom=fechafrom,fechato=fechato,numpag=5)
	else :	
...
def movimientosconvariables(cuentaid,fechafrom,fechato,numpag) :
	...
	r6=requests.get("https://apis.bbva.com/accounts-sbx/v1/me/accounts/"+cuentaid+"/transactions?dateFrom="+fechafrom+"&dateTo="+fechato+"&pageSize=5",headers=cabecerar6)
	json=r6.json()
	return json
